...
interface LessonWithDate extends Lesson {
  date: Date;
}

const UpcomingLessons: React.FC = () => {
  const { lessons, setLessons } = useLessons(); // Ensure `setLessons` is available in the context
  ...
  useEffect(() => {
    const fetchLessons = async () => {
      try {
        ...
        const lessonsWithDate = data.map((lesson) => ({
          ...lesson,
          date: new Date(lesson.date),
        })) as LessonWithDate[];

        setLessons(lessonsWithDate); // Update lessons in the context
        setError(null);
      } catch (err) {
                    ...
                    <div>
                      ...
                      <div className="flex items-center mt-1">
                        ...
                        <p className="text-xs text-gray-500">
                          {lesson.startTime || 'N/A'} - {lesson.endTime || 'N/A'} ({lesson.duration || 'N/A'} hr
                          {lesson.duration && lesson.duration !== 1 ? 's' : ''})
                        </p>
                      </div>
                    </div>
                  </div>
